  ...
  create<BehaviorStoreState>()(
    persist(
      (set) => ({
        ...
        incrementCounter: (id: number) => set((state) => ({
          behaviors: state.behaviors.map((behavior) =>
            behavior.id === id ? { ...behavior, counter: behavior.counter + 1 } : behavior
          ),
        })),
      }),
      {
...
export { useBehaviorColumnOneStore, useBehaviorColumnTwoStore, useTimeoutAllVariables }