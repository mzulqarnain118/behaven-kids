...
const CbsAddOrTransferClientsToRooms: React.FC<
  CbsAddOrTransferClientsToRooms
> = ({
  showModel,
  setShowModel,
  roomInfo,
  ...
  const [showManualTimeOutModal, setShowManualTimeOutModal] =
    useState<boolean>(false);

  if (!open) return null;
  const handleClose = async () => {
    try {
      await ApiCall("TimeOutRoom/AddLevelOnePerClient", "POST", null, {
        clientID,
        levelOneCounter: levelOneTotal,
      });
      setShowModel(false);
    } catch (error) {
  ...
  const transferToAnotherRoom = async (newRoomID: number) => {
    try {
      await ApiCall("Cbs/CbsChangesClientsToAWaitingRoom", "POST", null, {
        clientID,
        roomID: newRoomID,
        staffIDWhoTransferedClient: staffID,
        locationID,
        previousRoomID,
      });
      setShowModel(false);
    ...
    <>
      <div>
          ...
          <BootstrapModal.Body className="d-flex justify-content-center align-items-center">
            {clientProgram === "ABA" && (
              <div style={{ textAlign: "center" }}>
                <h4>
                  {clientFullName} - {clientProgram}
                </h4>
                <br />
                <div
                  style={{ textAlign: "center" }}
                  className="grid-container-for-room-selection"
                >
                  {roomInfo
                    .filter(
                      (info) =>
                        info.roomName.includes("RBT") &&
                        info.staffFirstName !== null
                    )
                    .map((info) => (
                      <button
                        key={info.roomID}
                ...
                <button
                  style={{ width: "150px" }}
                  className="round-button-for-choose-room grid-item-container-for-room-selection"
                  onClick={() => transferToAnotherRoom(34)}
                >
                  {" "}
                  RBT
                </button>
                ...
                >
                  ...
                  >
                    <span>
                      ...
                      ></img>
                      <span style={{ marginLeft: "5px" }}>Health</span>
                    </span>
                  </button>
                </div>
              </div>
            ...
            {clientProgram === "SDP" && (
              <div style={{ textAlign: "center" }}>
                ...
                >
                  {roomInfo
                    .filter(
                      (info) =>
                        !info.roomName.includes("RBT") &&
                        !info.roomName.includes("THR") &&
                        !info.roomName.includes("GS") &&
                        !info.roomName.includes("TOR") &&
                        info.roomName !== "None"
                    )
                    .map((info) => (
                      <button
                ...
                {setLevelOneTotal !== undefined && (
                  <div style={{ marginTop: "10px" }}>
                    <button
                      ...
                      className="round-button-for-choose-room grid-item-container-for-room-selection"
                      onClick={() =>
                        setLevelOneTotal((levelOneTotal) => levelOneTotal + 1)
                      }
                    >
                ...
                >
                  {roomInfo
                    .filter((info) => info.roomName.includes("TOR"))
                    .map((info) => (
                      <button
                        key={info.roomID}
                        style={{ width: "150px" }}
        ...
        {clientID !== null && clientFullName !== null && staffID !== null && (
          <ManuallyAddTimeoutModal
            showModel={showManualTimeOutModal}
            setShowModel={setShowManualTimeOutModal}
            clientID={clientID}
            clientFullName={clientFullName}
